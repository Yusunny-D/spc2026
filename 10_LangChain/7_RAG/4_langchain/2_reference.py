...
DB_DIR = './chroma'
COLLENTION_NAME = 'my_rag'

embeddings = OpenAIEmbeddings(model="text-embedding-3-small")

store = Chroma(collection_name=COLLENTION_NAME, embedding_function=embeddings, persist_directory=DB_DIR)
...
prompt = ChatPromptTemplate.from_messages([
    ("system", "당신은 문서 기반 QA시스템입니다. 아래 문서만 참고해서 답변하세요."
...
# 아래 코드에서 개별 답변 변호와 참고자료 번호 맞추기
# 그래서 중복 래퍼런스도 허용하기
# 이때, 프롬프트에도 명확하게 답변번호와 출처의 번호를 맞춰서 답변하시오.
def extract_source(docs): # unique 소스 출력
    seen, sources = set(), []
    for d in docs:
...
def retriever_and_split(inputs):
    ...
    return {
        ...
        "source": extract_source(docs)
    }

def append_source(d):
    src_lines = "\n".join(f' - {s}' for s in d['source'])
    return f"{d["answer"]}\n\n 참고문서: \n{src_lines}"
