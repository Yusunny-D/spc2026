# 1. openai 관련 라이브러리 다 불러온다.
# 2. 000 페이지(최종 페이지)에서 채팅창 FE를 만든다.
# 3. 그 form의 입력값을 be에서 post로 받아서 chatgpt api 호출
# 3-2 응답을 받아서 다시 fe에 반환해서 결과 출력한다. (+ SSE 구현해봐도 됨)
# 4. 우리의 이 상황(학년, 커리큘럼)에 대새서 영어로 대화 하도록 만든다.
