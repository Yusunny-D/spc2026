...
def remote_file(source):
    ...
    msg = f"'{source}' 삭제 완료" if existed else f"'{source}'는 목록에 없습니다."
...
def list_documents(): # 실무에서 절대 이렇게 안짜요...
    return [{"source": s, "chunks": c} for s, c in sorted(_distinct_source().items())]
...
@app.get('/files')
def files():
    # VectorDB(ChromaDB) 안에 있는 파일들 목록 가져오기
    # 
